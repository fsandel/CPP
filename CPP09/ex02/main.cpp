...
int getNewIndexWithJacobsthal(int originalIndex) {
  ...
  while (true) {
    ...
    if (jacobsthalNumber > originalIndex) return currentIndex - 1;
...
int main(int argc, char *argv[]) {
  try {
    ...
    //                                                                    argv);
    // PmergeMe<std::list<int>, std::list<std::pair<int, int> > > list(argc,
    // argv);
    ...
    // std::cout << "Size: " << argc - 1 << ",   Counter: " << counter
    //           << ",   Nln(N): " << (argc - 1) * log(argc - 1) << std::endl;
