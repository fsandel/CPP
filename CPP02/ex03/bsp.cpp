...
/*   Updated: 2023/03/14 20:17:57 by fsandel          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
bool bsp( Point const a, Point const b, Point const c, Point const p)
{
	...
	if (cross(a, b, p) || cross(a, c, p) || cross(b, c, p)) // check if point is on line
		return (false);

	return ((area1 + area2 + area3) <= area_full); // if point is outside: partial area is > than total	
}