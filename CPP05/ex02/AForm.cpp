...
AForm::AForm(std::string name, int req_grade_sign, int req_grade_exec)
  ...
  std::cout << "Trying to create form " << name << " with GradeToExec of "
            << req_grade_exec << " and GradeToSign of " << req_grade_sign
            << std::endl;
  if (req_grade_exec > 150 || req_grade_sign > 150)
    ...
    std::cout << this->_name << " with GradeToExec " << this->_req_grade_exec
              << " and GradeToSign " << this->_req_grade_sign << " got created"
              << std::endl;
  }
}
...
int AForm::getReqGradeSign(void) const { return (this->_req_grade_sign); }

int AForm::getReqGradeExec(void) const { return (this->_req_grade_exec); }

void AForm::beSigned(Bureaucrat bureaucrat) {
  std::cout << this->_name << " with GradeToSign " << this->_req_grade_sign
            << " tries to get signed by " << bureaucrat.getName()
            << " with grade of " << bureaucrat.getGrade() << std::endl;
  if (this->_signed == true)
    std::cout << this->_name << " is already signed" << std::endl;
  else if (bureaucrat.getGrade() > this->_req_grade_sign)
    std::cout << this->_name << " can't be signed because "
              << bureaucrat.getName() << "'s grade is too low" << std::endl;
  else {
...
std::ostream& operator<<(std::ostream& os, const AForm& obj) {
  if (obj.getSigned() == true) {
    os << obj.getName() << " is currently signed and"
       ...
       << " and a GradeToSign of " << obj.getReqGradeSign() << std::endl;
  } else {
...
void AForm::setSigned(const bool sign) { this->_signed = sign; }

void AForm::execute(Bureaucrat const& executor) const {
  std::cout << this->_name << " with GradeToExec " << this->_req_grade_exec
            << " is trying to get executed by " << executor.getName()
            << " with a grade of " << executor.getGrade() << std::endl;
  if (this->_signed == false)
    throw(AForm::FormNotSigned());
  else if (this->_req_grade_exec < executor.getGrade())
    throw(AForm::GradeTooLowException());
  else
    std::cout << executor.getName() << " can execute " << this->_name
              << std::endl;
}
