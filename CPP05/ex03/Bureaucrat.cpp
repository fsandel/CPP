...
{
	std::cout << this->_name << " with grade " << this->_grade << " is trying to execute " << form.getName() << " with GradeToExec of " << form.getReqGradeExec() << std::endl;
	if (this->_grade > form.getReqGradeExec())
		std::cout << this->_name << "'s grade is to low to execute " << form.getName() << std::endl;
	else if (form.getSigned() == false)
		std::cout << this->_name << " can't sign " << form.getName() << " because it's already signed" << std::endl;
	else
	{
		form.execute(*this);
